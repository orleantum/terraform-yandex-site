...
![Yandex Cloud](https://img.shields.io/badge/Yandex%20Cloud-FF0000.svg?style=for-the-badge&logo=yandex&logoColor=white)
...
Проект не просто создает бакет, а демонстрирует некоторые практики DevOps:
...
1. Сервисный аккаунт с правами `admin` на каталог и выпустить для него статические ключи (`access_key` и `secret_key`) + файл `key.json`.

2. Пустой приватный бакет для хранения стейта (например, `my-tfstate-bucket`).

3. Вписать имя этого бакета и статические ключи в блок `backend "s3"` в файле `default.tf`.
